...
void PigeonApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<PigeonApi> *api) {
  {
    ...
    if (api) {
      NSCAssert([api respondsToSelector:@selector(testMessageFromFlutter:error:)], @"PigeonApi api (%@) doesn't respond to @selector(testMessageFromFlutter:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSString *arg_fromFlutter = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
        Response *output = [api testMessageFromFlutter:arg_fromFlutter error:&error];
